...
class DanmarksRadio(object):
	...
	def get_direct_video_url(self, video_url):
		...
		if response is not None and 'Links' in response:
			...
			ios_links = []
			for link in links:
				if link['Target'] == 'Ios': # We are interested in finding links which are viewable on an iOS device
					ios_links.append(link)
			if len(ios_links) > 0:
				# We sort the list to find the link with the highest bitrate
				sorted_links = sorted(ios_links, key = operator.itemgetter('Bitrate'), reverse = True)
				return sorted_links[0]['Uri']
		return None
